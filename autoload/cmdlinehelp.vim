...
if !exists('g:cmdlinehelp#ignore')
    " Uninteresting stuff that should be ignored when searching for a command.
    let g:cmdlinehelp#ignore = 'sil\%[ent]\|verb\%[ose]\|debug' "{{{2
endif

if !exists('g:cmdlinehelp#patterns')
    ...
    " not on the command.
    " :nodefault:
    " :read: let g:cmdlinehelp#patterns = {}   "{{{2
    let g:cmdlinehelp#patterns = {
                \ 'set\?\s\+\zs\w\+': "'%s'",
                \ 'setl\%[ocal]\s\+\zs\w\+': "'%s'",
                \ 'let\s\+&l:\zs[^=[:space:]]\+': "'%s'",
                \ 'let\s\+&\zs[^=[:space:]]\+': "'%s'",
...
if !exists('g:cmdlinehelp#tags')
    " The tags. Defaults to standard help tags.
    let g:cmdlinehelp#tags = join(split(globpath(&rtp, 'doc/tags'), '\n'), ',') "{{{2
endif

if !exists('g:cmdlinehelp#table')
    ...
    " :nodefault:
    " :read: let g:cmdlinehelp#table = {} "{{{2
    let g:cmdlinehelp#table = {
                \ ':s\%[ubstitute]': ':s_flags',
                \ ':tag\?!': 'tag-!',
                \ ':Align': 'alignman',
                \ ':AlignCtrl': 'alignman',
...
if !exists('g:cmdlinehelp#prefixes')
    ...
    " |:helptags|.
    let g:cmdlinehelp#prefixes = ['cheat:']  "{{{2
endif
...
function! cmdlinehelp#View() "{{{3
    ...
    let ok = 0
    for [cpat, fmt] in items(g:cmdlinehelp#patterns)
        let tag = matchstr(s:buffer, '\(\('. g:cmdlinehelp#ignore .'\)\W*\s*\)*'. cpat)
    ...
    if !ok
        let tag = matchstr(s:buffer, '\(\('. g:cmdlinehelp#ignore .'\)\W*\s*\)*\zs\w\+!\?')
        let fmt = ':%s'
    endif
    " TLogVAR tag, fmt
...
function! s:TableGet(tag, default) "{{{3
    ...
    for [tag, repl] in items(g:cmdlinehelp#table)
        if a:tag =~ '^'. tag .'$'
            " TLogVAR tag, repl
            return repl
        endif
...
function! s:PrefixTag(tag) "{{{3
    for prefix in g:cmdlinehelp#prefixes
